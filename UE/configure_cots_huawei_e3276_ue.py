...
sys.path.append(os.path.expandvars('/packages/UE/'))
...
bandrich_ppd_config = '/packages/UE/conf/e392.conf'
...
def start_ue () :
   ...
   while 1:
     ...
     try:
        ...
        os.system ('ip route add 192.188.0.0/24 via ' + gw + ' dev ' + iface + ' 2> /dev/null')
...
def reset_ue():
  stringIdBandrich='Huawei Technologies Co., Ltd. Broadband stick (modem on)'
  status, out = commands.getstatusoutput('lsusb | grep -i \'' + stringIdBandrich + '\'')
  if (out == '') :
     print "Huawei E3276 LTE Adapter not found. Exiting now..."
     sys.exit()
  ...
  print "Huawei E3276 LTE Adapter found in..." + usb_dir
