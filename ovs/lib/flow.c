...
miniflow_extract(struct dp_packet *packet, struct miniflow *dst)
{
    ...
    if (OVS_LIKELY(!(nw_frag & FLOW_NW_FRAG_LATER))) {
		...
		if(udp->udp_src == htons(2152) && udp->udp_dst == htons(2152)) {
			/* We need the first 8 bytes of the GTP header */
			if(size >= UDP_HEADER_LEN + 8) {
				const void* udpdata = data;
				size_t sizeudpdata = size;

				data_pull(&udpdata, &sizeudpdata, UDP_HEADER_LEN);
				const struct gtpheader* gtp = udpdata;
				miniflow_push_be32(mf, gtp_teid, gtp->teid);
			}
		}
            }
