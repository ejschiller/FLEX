...
void
match_set_gtp_id(struct match *match, ovs_be32 gtp_id)
{
    match_set_gtp_id_masked(match, gtp_id, OVS_BE32_MAX);
}

void
match_set_gtp_id_masked(struct match *match, ovs_be32 gtp_id, ovs_be32 mask)
{
    match->wc.masks.gtp_teid = mask;
    match->flow.gtp_teid = gtp_id & mask;
}
...
match_format(const struct match *match, struct ds *s, int priority)
{
    ...
    if (is_ip_any(f) && f->nw_proto == IPPROTO_UDP && wc->masks.gtp_teid) {
